  ...
  <div v-if="isOpen" class="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-zinc-950/40 dark:bg-zinc-950/60 backdrop-blur-sm" @click.self="emit('close')">
    <div class="bg-surface border border-hairline w-full max-w-md overflow-hidden animate-in zoom-in duration-200 shadow-sm">
      ...
      <div class="px-6 py-5 border-b border-hairline flex items-center justify-between bg-canvas">
        ...
        <button @click="emit('close')" class="text-ink-muted hover:text-ink p-1 rounded-md hover:bg-surface-hover transition-colors cursor-pointer">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
        ...
        <div class="flex gap-3 pt-2">
          <button type="button" @click="emit('close')" class="flex-1 py-3 border border-hairline rounded-xl text-sm font-semibold text-ink-muted hover:bg-canvas transition-colors cursor-pointer">
            Cancelar
          </button>
          <button 
            type="submit" 
            :disabled="isSaving"
            class="flex-[1.5] py-3 bg-brand-primary hover:bg-brand-hover text-white rounded-xl font-bold text-sm transition-colors disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
          >
